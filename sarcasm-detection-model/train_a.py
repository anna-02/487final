...
def get_performance(net, loss_fn, data_loader, device, prediction_file='prediction.txt'):
    ...
    # sarcastic
    s_f1 = f1_score(y_true, y_pred, pos_label=1)
    s_precision = precision_score(
        y_true, y_pred, pos_label=1)
    s_recall = recall_score(y_true, y_pred, pos_label=1)

    # non-sarcastic
    ns_f1 = f1_score(y_true, y_pred, pos_label=0)
    ns_precision = precision_score(
        y_true, y_pred, pos_label=0)
    ns_recall = recall_score(y_true, y_pred, pos_label=0)

    precision = {'S_precision': s_precision, 'NS_precision': ns_precision}
    recall = {'S_recall': s_recall, 'NS_recall': ns_recall}
    f1 = {'S_f1': s_f1, 'NS_f1': ns_f1}
    # save predictions
